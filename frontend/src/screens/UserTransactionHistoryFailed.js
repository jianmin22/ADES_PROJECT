...
function UserTransactionHistoryFailed() {
  const { auth } = useAuth();
  const userId = auth.userId;
  const navigate = useNavigate();
  const axiosPrivate = useAxiosPrivate();
  const [failedTransactionHistory, setFailedTransactionHistory] = useState([]);
  ...
  useState("");
  const [failedTransactionFetched, setFailedTransactionFetched] =
    useState(false);
  ...
  useEffect(() => {
    const fetchFailedTransactionHistory = async () => {
      try {
        if (userId) {
          const response = await axiosPrivate.get(
            `/transactionHistory/${userId}/${currentPage}/failed`
          );
          ...
          setFailedTransactionHistory(transactionItems);
          setFailedTransactionFetched(true);
        } else {
  ...
  return (
    <div>
      {failedTransactionHistory.length === 0
        ? failedTransactionFetched && (
            <div className="h-screen flex flex-col justify-center items-center bg-orange-100">
              <h1 className="text-3xl font-bold mb-5">
                No failed transaction history
              </h1>
              <Link to="/shop/search">
        ...
        : failedTransactionFetched && (
            <div className="bg-orange-300 min-h-screen">
              <h1 className="text-3xl font-bold mx-10 mb-5 pt-5">
                Your Failed Transaction History
              </h1>
              {failedTransactionHistory.map((transaction) => (
                <div
                  key={transaction.transactionId}
                  className={`p-6 mb-6 mx-10  ${
                    transaction.state === "failed" ? "bg-gray-200" : "bg-white"
                  }`}
                >
                  <div className="flex items-center justify-end pb-2 mb-3 border-b border-gray-400">
                    ...
                    <div className="text-red-500 py-1 px-2 rounded mr-2">
                      Failed
                    </div>
                  </div>
...
export default UserTransactionHistoryFailed;
